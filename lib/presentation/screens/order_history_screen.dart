...
class _OrderHistoryScreenState extends State<OrderHistoryScreen> {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 12),
              Text(
                'Payment: ${order.paymentMethod == 'wallet' ? 'Ví điện tử' : 'COD'}',
                style: const TextStyle(
                  fontSize: 12,
                  color: Colors.grey,
                ),
  ...
  Widget _buildOrderActions(BuildContext context, OrderEntity order) {
        ...
        Expanded(
          child: SizedBox(
            ...
            child: ElevatedButton(
              onPressed: () async {
                await _confirmReceived(order.id);
                if (mounted) {
                  context.read<OrderHistoryProvider>().refreshOrders(widget.userId);
                }
              },
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Đơn hàng đã được xác nhận nhận hàng')),
        );
      }
    } catch (e) {
      if (mounted) {
