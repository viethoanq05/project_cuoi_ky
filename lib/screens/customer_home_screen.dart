...
class CustomerHomeScreen extends StatefulWidget {
  ...
  const CustomerHomeScreen({super.key, required this.authService});
...
class _CustomerHomeScreenState extends State<CustomerHomeScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final recommended = await _recommendationService
          .getWeatherBasedRecommendations(allStores, lat, lon)
          .catchError((e) => allStores);

      final foodSuggestions = await _recommendationService
          .getDistanceBasedFoodRecommendations(allFoods, allStores, lat, lon)
          .catchError((e) => allFoods.take(10).toList());

      final weatherRecommendations = await _recommendationService
          .getWeatherBasedFoodRecommendations(
            allFoods,
            weather,
          ...
          : RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  // Weather Widget
                  WeatherWidget(weather: _currentWeather, onRefresh: _loadData),
  ...
  void _applyFilters() {
    if (_selectedCategory == null) {
      ...
      debugPrint(
        'Filtering for category: $selectedId. Found: ${filteredByCategory.length} items out of ${_allFoods.length}',
      );
      ...
      _distanceSuggestions = foodWithDistance
          .map((e) => e.key)
          .take(50)
          .toList();
    }
                  ...
                  onPressed: () async {
                    ...
                    if (storeIndex != -6) {
                      if (storeIndex != -1) {
                        ...
                        storeName = await SearchService().getStoreNameById(
                          food.storeId,
                        );
                      }
                    }
